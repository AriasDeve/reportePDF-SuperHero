...
/* Procedure structure for procedure `GetMultipleRaces` */

/*!50003 DROP PROCEDURE IF EXISTS  `GetMultipleRaces` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `GetMultipleRaces`(IN _race_id VARCHAR(255))
BEGIN
    SELECT * FROM race WHERE race_id  = _race_id;
END */$$
DELIMITER ;

/* Procedure structure for procedure `spu_aligment_list` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_aligment_list` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_aligment_list`(
			IN _publisher_id int,
			in _alignment_id int
		)
begin
			select sup.id, sup.superhero_name, sup.full_name, 
					ra.race, pu.publisher_name
			 from superhero sup
			INNER JOIN race ra ON ra.`id` = sup.race_id
			INNER JOIN publisher pu ON pu.id = sup.publisher_id
			WHERE sup.alignment_id = _alignment_id AND
					sup.publisher_id = _publisher_id			
			ORDER BY sup.id;
		END */$$
DELIMITER ;

/* Procedure structure for procedure `spu_alignment_graphic` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_alignment_graphic` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_alignment_graphic`()
BEGIN
	SELECT	alignment.`id`,
				alignment.`alignment`,
				COUNT(*) 'amount'
...
BEGIN
			SELECT * FROM alignment ORDER BY alignment.`alignment`;
		END */$$
DELIMITER ;
...
/* Procedure structure for procedure `spu_filtrar_superheros` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_filtrar_superheros` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_filtrar_superheros`(
	IN _race_id INT, 
	IN _gender_id INT,
	IN _alignment_id INT
)
...
/* Procedure structure for procedure `spu_listar_eyecolour` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_listar_eyecolour` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_listar_eyecolour`()
BEGIN
select 
		publisher.`id`,
		publisher.`publisher_name`,
...
/* Procedure structure for procedure `spu_listar_faction` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_listar_faction` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_listar_faction`(IN _publisher_id INT)
BEGIN
select 
		alignment.`id`,
		alignment.`alignment`,
...
/* Procedure structure for procedure `spu_listar_factionpublisherid` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_listar_factionpublisherid` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_listar_factionpublisherid`()
BEGIN
SELECT 
		publisher.`publisher_name`,
		alignment.`alignment`,
	...
	FROM superhero
		LEFT JOIN publisher ON publisher.`id` = superhero.`publisher_id`
		left join alignment on alignment.`id`= superhero.`alignment_id`
		WHERE publisher.`id` IN ('2','4','11','18','24')
		GROUP BY publisher.`publisher_name`, alignment.`alignment`
		ORDER BY publisher.`publisher_name`;
END */$$
DELIMITER ;

/* Procedure structure for procedure `spu_listar_superheros` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_listar_superheros` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_listar_superheros`(IN _publisher_id INT)
BEGIN
	SELECT
		superhero.`id`,
		superhero.`superhero_name`,
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_superhero_list_publisher`(
			IN _publisher_id INT
...
BEGIN
			SELECT sup.`id`, sup.`superhero_name`, sup.`full_name`, 
						...
						d.`colour` AS 'hair_colour',
						e.`colour` AS 'skin_colour'
				FROM superhero sup
			INNER JOIN colour c ON c.id = sup.`eye_colour_id` 
			INNER JOIN colour d ON d.`id` = sup.`hair_colour_id`
			INNER JOIN colour e ON e.`id` = sup.`skin_colour_id`
			WHERE sup.publisher_id = _publisher_id
			ORDER BY sup.`id`;
		end */$$
DELIMITER ;

/* Procedure structure for procedure `spu_superhero_multi_race` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_superhero_multi_race` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_superhero_multi_race`( 
...
/* Procedure structure for procedure `spu_superhero_multi_race2` */

/*!50003 DROP PROCEDURE IF EXISTS  `spu_superhero_multi_race2` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_superhero_multi_race2`(IN _race_id VARCHAR(255))
BEGIN
    SET @sql = CONCAT('SELECT * FROM race WHERE FIND_IN_SET(id, ?) > 0');
    PREPARE stmt FROM @sql;
    EXECUTE stmt USING _race_id;
